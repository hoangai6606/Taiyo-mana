...
import { Shield, Search, Filter, Clock, User, ArrowRight, Eye, RefreshCw } from 'lucide-react';
...
export default function AuditLogPage() {
  ...
  }, [activeTab, isAuthorized, loadAuditLogs, loadApprovalLogs]);
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full p-8 text-center">
        ...
        <p className="text-slate-500 max-w-sm">
          Bạn không có quyền truy cập trang này. Chỉ Manager và Accounting Admin mới có thể xem nhật ký hệ thống.
        </p>
      </div>
    );
  }
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100">
        <div className="flex items-center justify-between px-6 pt-4 pb-0 border-b border-slate-100">
          ...
          <div className="flex items-center gap-2 pb-2">
            ...
            <button
              onClick={() => activeTab === 'audit' ? loadAuditLogs() : loadApprovalLogs()}
              disabled={loading}
              className="flex items-center gap-1.5 px-3 py-1.5 text-sm text-slate-600 hover:text-slate-900 border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors disabled:opacity-50"
            >
                ...
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={7} className="px-4 py-16 text-center">
                      ...
                      <p className="text-slate-400 text-sm">
                        {searchQuery ? 'Không tìm thấy kết quả phù hợp' : 'Chưa có dữ liệu audit log'}
                      </p>
                    </td>
                  </tr>
                ) : (
                  filteredAuditLogs.map(log => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <Eye className="w-4 h-4 text-slate-300 hover:text-slate-600 transition-colors" />
                      </td>
                    </tr>
                  ))
                )}
        ...
        {activeTab === 'approval' && (
          <div className="overflow-x-auto">
            ...
            {!loading && approvalLogs.length > 0 && (
              <div className="px-4 py-3 border-t border-slate-100 text-xs text-slate-400">
                Hiển thị {approvalLogs.length} bản ghi
              </div>
            )}
          </div>
        )}
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <span className="px-2.5 py-1 bg-slate-100 text-slate-700 rounded font-medium text-sm">
